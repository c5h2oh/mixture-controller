...
//adjust for temp and pressure  
    adjusted_pulse = channel_pulse * (reference_temp/(bmp.readTemperature()+273)*(bmp.readPressure()/reference_pressure)); 
    adjusted_pulse = constrain(adjusted_pulse, 1000, 2000);
    solenoid_pulse = map(adjusted_pulse, 1000, 2000, 6145, 0); 
...
//print channel pulse
  sprintf(_buffer, "%04u \x0e5""S Channel Pulse", (int)(channel));
...
sportData getSensorData2(CustomSPortSensor* compensation_ratio) {
  ...
  data.value = int(1000 * adjusted_pulse/channel_pulse);                      //Set the sensor value 
  return data;
}
